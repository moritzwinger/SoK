...
void ptxt_weights_enc_input_rnn(const seal::GaloisKeys &galois_keys, seal::Evaluator &evaluator,
                                seal::CKKSEncoder &encoder, size_t dim, std::vector<vec> diagonals_W_x,
                                std::vector<vec> diagonals_W_h, vec b,
 ...
 */
bool decrypt_and_compare(const seal::Ciphertext &ctxt_r, vec expected, seal::Decryptor &decryptor,
                         seal::CKKSEncoder &encoder, float tolerance = 0.001);